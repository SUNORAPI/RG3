...
public class Charter : MonoBehaviour
{
    ...
    private void Awake()
    {
        ...
        var json = Resources.Load<TextAsset>(_Filepath).text;//ƒoƒO‚ ‚è
    ...
    void Judge(Touch T)
    {
        List<note> JNL = Notes.Where((note => T.MusicTime < note.Movestarttime + NoteMove.Movetime + 0.1 || T.MusicTime > note.Movestarttime - 0.1)).ToList();
        foreach (note note in JNL)
        {
